    ...
    try:
        ...
        for i in range(0, len(scenarios_chunk), batch_size):
            ...
            if i % 2 == 0 :
                ...
                print(f"Processing scenarios {i} to {i + batch_size} of {len(scenarios_chunk)}")
            ...
            if i % 2 == 0:
                map_cache_path = os.path.join(os.getcwd(), 'map_cache')
                if os.path.exists(map_cache_path):
                    import shutil
                    shutil.rmtree(map_cache_path)
                    print(f"Cleared map cache at {map_cache_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--nuplan_train', type=str, default='./nuplan_train_json/nuplan_train_chunk_20.json')
